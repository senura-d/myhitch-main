...
export default function AuthForm() {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto bg-white/30 backdrop-blur-md rounded-3xl border border-black/15 shadow-2xl overflow-hidden grid md:grid-cols-12 min-h-[580px]">
      
      {/* 1. LEFT BRANDING PANEL (hidden on mobile) */}
      <div className="hidden md:flex md:col-span-5 bg-gradient-to-br from-[#001B48] to-[#02457A] p-8 flex-col justify-between relative overflow-hidden text-white border-r border-black/15">
        {/* Glow Effects */}
        <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_top_left,rgba(214,232,238,0.15),transparent_60%)]" />
        <div className="absolute -bottom-20 -left-20 w-44 h-44 bg-teal-500/10 rounded-full blur-2xl" />

        {/* Header */}
        <div className="relative z-10">
          <Link 
            href="/" 
            className="inline-flex items-center gap-2 text-white/80 hover:text-white transition-colors text-sm font-semibold mb-8 group"
          >
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <span className="font-display font-bold tracking-tight text-xl">MYHitch</span>
          </div>
        </div>
        ...
        <div className="my-auto relative z-10 py-6">
          <div className="mb-6 relative flex items-center justify-center h-28 w-28 mx-auto">
            ...
            <motion.div 
              className="absolute inset-0 rounded-full border border-[#D6E8EE]/20"
              animate={{ rotate: 360 }}
              transition={{ repeat: Infinity, duration: 12, ease: "linear" }}
            />
            <motion.div 
              className="absolute inset-2 rounded-full border border-dashed border-[#D6E8EE]/30"
              animate={{ rotate: -360 }}
              transition={{ repeat: Infinity, duration: 8, ease: "linear" }}
            />
            <div className="h-10 w-10 rounded-xl bg-white/10 border border-white/25 flex items-center justify-center text-white">
              <ShieldCheck size={20} />
            </div>

            {/* Orbiting items */}
            <div className="absolute top-0 left-1/2 -translate-x-1/2 -mt-2">
              <span className="h-6 w-6 rounded-full bg-[#018ABE] flex items-center justify-center text-[10px] shadow-md border border-white/10"><ShoppingBag size={10} /></span>
            </div>
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 -mb-2">
              <span className="h-6 w-6 rounded-full bg-emerald-600 flex items-center justify-center text-[10px] shadow-md border border-white/10"><CheckCircle2 size={10} /></span>
            </div>
            <div className="absolute right-0 top-1/2 -translate-y-1/2 -mr-2">
              <span className="h-6 w-6 rounded-full bg-[#001B48] flex items-center justify-center text-[10px] shadow-md border border-white/10"><Globe size={10} /></span>
            </div>
            <div className="absolute left-0 top-1/2 -translate-y-1/2 -ml-2">
              <span className="h-6 w-6 rounded-full bg-[#02457A] flex items-center justify-center text-[10px] shadow-md border border-white/10"><Compass size={10} /></span>
            </div>
          </div>

          <h3 className="font-display text-xl font-bold text-center leading-tight mb-2">
            One Secure Account.
          </h3>
          <p className="text-xs text-center text-slate-300 leading-relaxed max-w-[240px] mx-auto">
      ...
      <div className="col-span-12 md:col-span-7 p-6 sm:p-10 flex flex-col justify-between bg-white/20">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="bg-black/5 p-1 rounded-full flex relative z-10 border border-black/5">
            <button 
            ...
            >
              {mode === "signin" && (
                <motion.span 
                  ...
                  className="absolute inset-0 bg-[#001B48] rounded-full -z-10"
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full bg-white/40 border border-black/15 text-black placeholder-slate-600 text-sm rounded-xl pl-10 pr-4 py-3 outline-none focus:border-black/30 focus:bg-white/60 transition-all"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between items-center">
                      ...
                      <a href="#forgot" className="text-xs text-[#018ABE] hover:underline font-semibold">Forgot?</a>
                    </div>
                    <div className="relative">
                      ...
                      <input 
                        ...
                        onChange={handleInputChange}
                        placeholder="••••••••"
                        className="w-full bg-white/40 border border-black/15 text-black placeholder-slate-600 text-sm rounded-xl pl-10 pr-10 py-3 outline-none focus:border-black/30 focus:bg-white/60 transition-all"
                      />
                      <button 
                        type="button"
                  ...
                  <div className="flex items-center gap-2 py-1">
                    <input 
                      ...
                      onChange={handleInputChange}
                      className="rounded border-black/20 text-[#001B48] focus:ring-0 focus:ring-offset-0 cursor-pointer h-4 w-4 bg-white/40"
                    />
                  ...
                  <button 
                    type="submit"
                    disabled={loading}
                    className="w-full btn btn-primary flex items-center justify-center gap-2 rounded-xl py-3.5 text-sm font-bold shadow-md shadow-black/10 cursor-pointer disabled:opacity-50"
                  >
                    {loading ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="flex items-start gap-2 py-1">
                    <input 
                      ...
                      onChange={handleInputChange}
                      className="rounded border-black/20 text-[#001B48] focus:ring-0 focus:ring-offset-0 cursor-pointer h-4 w-4 bg-white/40 mt-0.5"
                    />
                    <label htmlFor="agreeTerms" className="text-xs text-slate-700 font-medium select-none cursor-pointer">
                      I agree to the <a href="#terms" className="text-[#018ABE] hover:underline font-semibold">Terms of Service</a> and <a href="#privacy" className="text-[#018ABE] hover:underline font-semibold">Privacy Policy</a>
                    </label>
                  </div>
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-2 gap-3 mt-4">
            <button className="flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl border border-black/15 bg-white/40 hover:bg-white/60 transition-colors text-xs font-bold text-black cursor-pointer shadow-sm">
